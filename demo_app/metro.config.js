...
const demo_appNodeModules = path.resolve(__dirname, 'node_modules');
...
module.exports = {
  ...
  resolver: {
    ...
    // Prefer demo_app/node_modules for package resolution.
    nodeModulesPaths: [demo_appNodeModules],
    ...
    resolveRequest: (context, moduleName, platform) => {
      ...
      //    to demo_app/node_modules to prevent duplicate packages.
      if (result?.filePath) {
        const fp = norm(result.filePath);
        if (fp.startsWith(parentNm + '/')) {
          const relative = fp.slice(parentNm.length + 1);
          const redirected = path.resolve(demo_appNodeModules, relative);
